...
static esp_err_t configure_spi_dev(slot_info_t *slot, int clock_speed_hz)
{
    ...
    spi_device_interface_config_t devcfg = {
        ...
        // CYD2USB port: SPI mode 3, NOT the IDF-default mode 0. On the
        // ESP32-2432S028R (two-USB "CYD2USB") boards, the SD slot's clock path
        // makes mode-0 sampling marginal — CMD0/CMD8 respond but ACMD41 never
        // completes (card stays in idle, mount fails with ESP_ERR_NOT_SUPPORTED
        // 0x106). SD spec allows both SPI modes 0 and 3; mode 3 (CPOL=1,CPHA=1)
        // initialises the same cards cleanly (verified with a raw-CMD probe).
        .mode = 3,
