...
esp_err_t sdmmc_init_spi_crc(sdmmc_card_t* card)
{
    ...
    if (err == ESP_ERR_NOT_SUPPORTED) {
        /* CYD2USB port: some (cheap/clone) SD cards do not implement CMD59
         * (CRC_ON_OFF) and answer "illegal command". The SD SPI spec makes
         * CRC16 optional, so we just continue with CRC disabled — the sdspi
         * driver skips data-CRC verification when CMD59 never succeeded.
         * Treating this as fatal would make these cards unmountable.
         */
        ESP_LOGW(TAG, "%s: card rejected CMD59 (CRC_ON_OFF) — continuing without data CRC16", __func__);
        return ESP_OK;
    }
    if (err != ESP_OK) {
