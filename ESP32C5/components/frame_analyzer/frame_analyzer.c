...
void frame_analyzer_capture_stop(){
    // Use normal error checking instead of ESP_ERROR_CHECK to avoid crash if handler not registered
    esp_err_t err = esp_event_handler_unregister(SNIFFER_EVENTS, SNIFFER_EVENT_CAPTURED_DATA, &data_frame_handler);
    if (err != ESP_OK && err != ESP_ERR_NOT_FOUND) {
        ESP_LOGW(TAG, "Failed to unregister data frame handler: %s", esp_err_to_name(err));
    }
}
