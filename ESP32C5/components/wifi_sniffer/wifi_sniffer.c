...
static const uint8_t channel_list[] = {1, 6, 11, 2, 7, 3, 8, 4, 9, 5, 10, 12, 13};
...
esp_err_t wifi_sniffer_stop(void) {
    ...
    if (sniffer_aps != NULL) {
        heap_caps_free(sniffer_aps);
        sniffer_aps = NULL;
        ESP_LOGI(TAG, "Freed sniffer_aps from PSRAM");
    }
    
    if (probe_requests != NULL) {
        heap_caps_free(probe_requests);
        probe_requests = NULL;
        ESP_LOGI(TAG, "Freed probe_requests from PSRAM");
    }
