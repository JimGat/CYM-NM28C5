...
uint8_t *pcap_serializer_init(){
    ...
    pcap_buffer = (uint8_t *)heap_caps_malloc(sizeof(pcap_global_header_t), MALLOC_CAP_SPIRAM);
    if (pcap_buffer == NULL) {
        ESP_LOGE(TAG, "Failed to allocate PCAP buffer in PSRAM");
        return NULL;
    }
    
    pcap_size = sizeof(pcap_global_header_t);
    memcpy(pcap_buffer, &pcap_global_header, sizeof(pcap_global_header_t));
    ESP_LOGI(TAG, "PCAP serializer initialized in PSRAM (%u bytes)", pcap_size);
    return pcap_buffer;
}
