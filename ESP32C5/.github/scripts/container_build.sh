...
Builds the ESP32-C5 firmware with ESP-IDF release-v6.0.
Defaults to running inside Docker (espressif/idf:release-v6.0).

Options:
  --no-docker   Run idf.py directly (for GitHub Actions container jobs or a host with ESP-IDF installed)
EOF
}
