...
4. Build from `ESP32C5` with ESP-IDF loaded:
   ...
   - `idf.py build`
5. The CMake post-build hook copies firmware into `ESP32C5/binaries-esp32c5/` and generates the merged full image; verify all four tracked binaries changed.
6. Before commit, verify the binary contains the expected version:
   ...
   - `strings ESP32C5/binaries-esp32c5/CYM-NM28C5-full.bin | grep vX.Y.Z`
...
`CYM-NM28C5-full.bin` is a merged flat image (bootloader + partition table + firmware) generated by the CMake post-build hook. Flash it at address `0x0000` with any flasher that accepts a single full binary. Never create a release without all four files attached.
